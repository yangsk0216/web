<?php 
	...
	//Define sql query
	$query = "SELECT id, state, location, address, latitude, longitude FROM fandian ";
	//Execute query
	...
	while ($row = pg_fetch_array($result, null, PGSQL_ASSOC))	{
		...
		$fandian[] = array("id" => $row["id"], "state" => $row["state"],"loc" => $row["location"],"add" => $row["address"],"lat" => $row["latitude"], "lon" => $row["longitude"]);
	}
	
	//Encode tweetData array in JSON
	echo json_encode($fandian); 
